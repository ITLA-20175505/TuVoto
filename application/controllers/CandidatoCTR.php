...
class CandidatoCTR extends CI_Controller {
	...
	public function Editar($id = 0){
		...
		if(count($Candidato) > 0){
			...
			$this->load->view('FormCandidato',['persona'=>$Candidato,'error'=>false,'confirmacion'=>false]);
			pie::aplicar();
			if($_POST){
				if(isset($_POST['ConsultaCedula']) && $_POST['ConsultaCedula']!= ""){
					$cedula = $_POST['ConsultaCedula'];
			...
			$urlCandidato = base_url('index.php/CandidatoCTR');
			echo "<script>alert('No Existe el Candidato')
			window.location = '{$urlCandidato}';</script>";
		}
	}
