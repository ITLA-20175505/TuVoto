...
$titulares =nivel_model::nombre_niveles_x_Eleccion($eleccionActiva['IdEleccion']);
// var_dump($_POST);
$urlVotacion = base_url('index.php/VotacionCTR/imprimirVoto/').$IdVotacion;
...
<td>
  ...
  <section class="my-5 bg-dark" style="min-height: 640px">
	<div class="container  bg-dark">
		...
		<ol class="carousel-indicators" id="sliders"></ol>
...
SLIDE;
					...
					foreach($casillas as $ckey =>$casilla){	
						AsgCasilla($casilla,$idNiveles,$titulares);
					}
					echo"</div></div>";
	...
	<button class="btn btn-lg btn-info btn-block" style="height: 100px" type="button" onclick="guardar()"> PRESIONE AQUI SI Y SOLO SI HA TERMINADO DE VOTAR</button>	 
	</div>
...
<script>
	...
	function guardar(){
		...
		if(nose){
			confirmarVotacion('Enviar Votacion','Esta Seguro que desea enviar los votos?'
			,'question','Enviar Votos','No! Volver Atras','<?=$urlVotacion?>');
		}else{
...
function nuevo_detalle(nivel,candidato,niveles,titulos){
	...
	if(lista_votos.length < niveles.length){
		for(i=0;i<niveles.length;i++){
			...
			votos.IdCandidato = 0;
			votos.titulo = titulos[i].Nombre;
			lista_votos.push(votos);
		}	
	}
	console.log(lista_votos);
	voto ={};
	voto.IdNivel = nivel;
	...
	for (i = 0; i < lista_votos.length; i++) {
		if(lista_votos[i].IdNivel == voto.IdNivel){
			...
			lista_votos[i].titulo = voto.titulo;
			$(' #carouselSixColumn'). carousel( i+1);
			document.getElementById('titulo').innerHTML = lista_votos[i+1].titulo;
		}
	}
